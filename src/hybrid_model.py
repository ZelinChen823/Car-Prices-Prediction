...
baseline_rmse = np.sqrt(mean_squared_error(df.loc[y_test_log.index, target], y_test_pred))
baseline_mae = mean_absolute_error(df.loc[y_test_log.index, target], y_test_pred)
baseline_r2 = r2_score(df.loc[y_test_log.index, target], y_test_pred)
print("Baseline XGBoost Model Performance (Real Target):")
print(f"RMSE: {baseline_rmse:.4f}, MAE: {baseline_mae:.4f}, R²: {baseline_r2:.4f}")
...
ensemble_predictions = np.exp(ensemble_predictions_log)

ensemble_rmse = np.sqrt(mean_squared_error(df.loc[y_test_log.index, target], ensemble_predictions))
ensemble_mae = mean_absolute_error(df.loc[y_test_log.index, target], ensemble_predictions)
ensemble_r2 = r2_score(df.loc[y_test_log.index, target], ensemble_predictions)
print("Ensemble Model Performance (Real Target):")
